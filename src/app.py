...
from core.ai_generator import GeminiEmailGenerator
load_dotenv()

st.set_page_config(page_title="Bulk Email Sender", layout="centered")
st.title(" Bulk Email Sender with AI")

# Initialize Session State for AI content
...
# --- NEW: Simplified AI Assistant Expander ---
with st.expander("AI Email Assistant (Generate a Draft)"):
    user_prompt = st.text_area(
        "Enter your prompt here:",
        placeholder="e.g., Write a professional email to a hiring manager asking for an AI internship. Mention I'm a student with Python and ML skills.",
        height=100
    )
        ...
        if not user_prompt:
            ...
            try:
                with st.spinner("Gemini is thinking... Please wait."):
                    ...
                    # Pass the single user prompt to the updated function
                    email_content = ai_generator.generate_email(user_prompt)
                    st.session_state.generated_subject = email_content['subject']
                    st.session_state.generated_body = email_content['body']
                    st.success("AI content generated! You can now edit it below.")
            except (ValueError, RuntimeError) as e:
                st.error(f"AI Generation Failed: {e}")

# --- Email Editor (Subject and Body) ---
...
email_body = st_quill(
    ...
    placeholder="Compose your email here, or generate one with the AI assistant above.",
    html=True,
    toolbar=[
...
st.header("Send It!")
# ... (Sending logic remains exactly the same) ...
...
if st.button("SEND EMAILS", use_container_width=True):
    ...
    if not sender_email or not sender_password:
        st.error("Sender credentials are required in the sidebar.")
    elif not recipients:
        st.warning("No recipients have been added.")
    elif not email_subject:
        st.warning("Email subject is empty.")
    elif not email_body or email_body == "<p><br></p>":
